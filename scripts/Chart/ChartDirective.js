  ...
  angular.module('app.chart.directives', []).directive('gaugeChart', [
    ...
    function() {
      return {
        ...
        link: function(scope, ele, attrs) {
          ...
          data = scope.data;
          options = scope.options;
          return scope.plot = $.plot(ele[0], data, options);
        }
      };
    }
    ...
    function() {
      return {
        ...
        link: function(scope, ele, attrs) {
          var chart = null;
          scope.setData = function(data, type){
                scope.chart.options.xkey = data.xkey;
                scope.chart.options.ykeys = data.ykeys;
                scope.chart.options.labels = data.labels;
                scope.chart.setData(data.items);
                scope.chart.redraw();
                console.log(data);
                /* options = {
                    element: ele[0],
                    data: data.items,
                    xkey: data.xkey,
                    ykeys: JSON.parse(data.ykeys),
                    labels: JSON.parse(data.ykeys),
          ...
          switch (attrs.type) {
            case 'line':
              ...
              options = {
                ...
                scope.chart = new Morris.Line(options);
              window._temp = scope.chart;
                  return scope.chart;
            case 'area':
              if (attrs.lineColors === void 0 || attrs.lineColors === '') {
                colors = null;
              ...
              return scope.chart = new Morris.Area(options);
            case 'bar':
              ...
              return scope.chart = new Morris.Bar(options);
            case 'donut':
